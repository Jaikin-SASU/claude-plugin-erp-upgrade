...
3. **Shortlist 2–4 candidates** with `references/selection-criteria.md`. For each vendor, read `${CLAUDE_PLUGIN_ROOT}/references/vendors/<vendor>.md` when it exists (see `${CLAUDE_PLUGIN_ROOT}/references/vendors/INDEX.md` for the file of each ERP); use only facts found there or verified live with URL and date. Never quote a price that is not published by the vendor; otherwise write "on quote". If the `erp-facts` MCP tools are available, call `vendor_facts`, `support_deadlines` or `einvoicing_status` first: same sourced facts, kept up to date.
